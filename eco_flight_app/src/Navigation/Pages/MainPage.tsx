...
import { Button, StyleSheet, Text, TouchableHighlight, View, Image, TouchableOpacity, ScrollView } from "react-native";
import { AIRPLANEFORSEARCH, ECOPLANETREE, FLIGHTBLACK, FLIGTHICON } from "../../helpers/images";
...
export default function MainPage({ navigation }: MainPageProps) {
  ...
  useEffect(() => {
    async function getMostRecentFlightInfo() {
      if (user) {
        await API.getMostRecentFlightOfUser(user.id).then((resultingFlight) => {
      ...
      <ScrollView style={{ marginTop: 15 }} contentContainerStyle={{ flex: 1 }}>
        ...
        <View style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 15 }}>
          <TouchableOpacity style={{
            ...
            borderRadius: 50, backgroundColor: GREEN, width: 70, height: 70, borderWidth: 1, borderColor: 'black'
          }}
            onPress={() => navigation.navigate("Search Flight")}
          >
            <Image style={{ width: 30, height: 30, transform: [{ rotate: "90deg" }] }} source={FLIGHTBLACK} />
          </TouchableOpacity>
        ...
        <TouchableOpacity onPress={() => navigation.navigate("Search Flight")} >
          <View style={{
            width: '90%', height: 450, borderRadius: 12, alignItems: 'center', justifyContent: 'center',
            marginLeft: 'auto', marginRight: 'auto'
          }}>

            <Image style={{ width: '100%', height: '100%', borderRadius: 12, alignItems: 'center', marginLeft: 'auto' }} source={ECOPLANETREE} />
